/**
 * @fileoverview Class encapsulating an object with physics
...
ObjectWithPhysics.createRandom = function() {
  return new ObjectWithPhysics(100, 100, -1, 1, null, null, 10, 10, "green");
};
...
ObjectWithPhysics.prototype.collision = function(otherObjects) {
  for (object in otherObjects) {
    if (Math.pow(object.getX() - this.x, 2) + Math.pow(object.getY - this.y, 2) < Math.pow(this.radius, 2)) {
      ...
      objectOutVelocity = Math.sqrt( totalOutEnergy * object.getMass() / (object.getMass() + this.mass));
      thisOutVelocity = Math.sqrt( totalOutEnergy * this.mass / (this.mass + object.getMass()));
 ...
 */
ObjectWithPhysics.prototype.update = function(otherObjects) {
